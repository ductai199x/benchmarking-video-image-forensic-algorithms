...
class SpatialAttentionModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # feed lage batch into the backbone to produce their features
        bb = self.backbone(patches)
        # split large batch back into embedded images
        bb = bb.contiguous().view(B, -1, self.embed_dim)
        # get the maps from the long distance attention
        lda_maps = self.long_dist_attn(bb)
        # scale the embeddings by the attention maps
        scaled_bb = torch.einsum("ijk,ilj->iljk", bb, lda_maps)
        # scaled_bb = scaled_bb.contiguous().view(B, -1)
        scaled_bb = torch.einsum("iljk->ijk", scaled_bb)

        # feed the scaled embeddings to a classifier to get the output
        class_label = self.classifier(scaled_bb)
        patch_label = self.localizer(
            scaled_bb.permute(0, 2, 1).unsqueeze(-1)
        )  # for localization
        patch_label = patch_label.squeeze()

        return class_label, patch_label


class SpatialAttnPLWrapper(LightningModule):
    def __init__(self, **config):
        super().__init__()

        self.model = SpatialAttentionModel(**config)
    ...
    def test_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            if y.sum() > 0:
                ...
                patch_preds = [
                    PatchPredictions(pl, self.patch_size, self.img_size)
