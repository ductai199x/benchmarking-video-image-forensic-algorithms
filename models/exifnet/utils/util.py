...
def random_resize(im_a, im_b, same):
    ...
    if same:
        if np.random.randint(2):
            a_par = get_param()
            im_a = cv2.resize(
                im_a, None, fx=a_par[0], fy=a_par[1], interpolation=a_par[2]
            )
            im_b = cv2.resize(
                im_b, None, fx=a_par[0], fy=a_par[1], interpolation=a_par[2]
            )
    else:
        a_par = get_param()
        ...
        if np.random.randint(2):
            ...
            while np.all(a_par == b_par):
                b_par = get_param()
            im_b = cv2.resize(
                im_b, None, fx=b_par[0], fy=b_par[1], interpolation=b_par[2]
            )
    return im_a, im_b
...
def random_jpeg(im_a, im_b, same):
    ...
    if same:
        if np.random.randint(2):
            a_par = get_param()
            _, enc_a = cv2.imencode(
                ".jpg", im_a, [int(cv2.IMWRITE_JPEG_QUALITY), a_par]
            )
            im_a = cv2.imdecode(enc_a, 1)
            _, enc_b = cv2.imencode(
                ".jpg", im_b, [int(cv2.IMWRITE_JPEG_QUALITY), a_par]
            )
        ...
        if np.random.randint(2):
            ...
            while np.all(a_par == b_par):
                b_par = get_param()
            _, enc_b = cv2.imencode(
                ".jpg", im_b, [int(cv2.IMWRITE_JPEG_QUALITY), b_par]
            )
            im_b = cv2.imdecode(enc_b, 1)
    return im_a, im_b
