...
class EfficientBenchmark:
    def __init__(
        ...
        self.label_shape = (
            1
            if not "num_classes" in net_module_obj_init_params
            else net_module_obj_init_params["num_classes"]
        )

        self.cr = self.update_queue_runner(im)
        self.auto_close_sess = auto_close_sess
        self.n_responses = (
            self.max_h_ind * self.max_w_ind if self.dense_compute else n_anchors
        )
    ...
    def update_queue_runner(self, im):
        ...
        cr = queue_runner.CustomRunner(
            fn, n_processes=self.num_processes, n_threads=self.num_threads
        )
        ...
        def new_cr(batch_size):
            (
                ...
                im_a,
                im_b,
            ) = self.original_cr_get_inputs(batch_size)
            ...
            if self.label_shape == 1:
                ...
                return (
                    im_a,
                    im_b,
                    tf.zeros((batch_size, self.label_shape), dtype=tf.float32),
                )
    ...
    def reset_image(self, im):
        ...
        while True:
            self.solver.sess.run(
                self.cr.tf_queue.dequeue_up_to(self.cr.tf_queue.size())
            )
    ...
    def get_anchor_patches(self):
        ...
        if self.dense_compute:
            ...
            return util.process_im(
                np.array(
                    [
                        self.get_patch(self.anchor_inds[i][0], self.anchor_inds[i][1])
                        for i in range(
                            self.anchor_count,
                            min(
        ...
        return util.process_im(
            np.array(
                [self.rand_patch() for i in range(self.n_anchors)], dtype=np.float32
            )
        )
    ...
    def set_image(self, image):
        ...
        self.image = image
        self.max_h_ind = 1 + int(
            np.floor((self.image.shape[0] - self.patch_size) / float(self.stride))
        )
        self.max_w_ind = 1 + int(
            np.floor((self.image.shape[1] - self.patch_size) / float(self.stride))
        )
        self.indices = (
            np.mgrid[0 : self.max_h_ind, 0 : self.max_w_ind].reshape((2, -1)).T
        )  # (n 2)
    ...
    def data_fn(self, hind, wind):
        ...
        patch = self.image[
            y_ind : y_ind + self.patch_size, x_ind : x_ind + self.patch_size, :
        ]
        ...
        if self.mirror_pred:
            ...
            batch_a, batch_b = np.vstack([batch_a, batch_b]), np.vstack(
                [batch_b, batch_a]
            )
    ...
    def argless_extract_inds(self):
        ...
        while True:
            ...
            yield iterator[
                count * self.max_bs : (count + 1) * self.max_bs, :
            ]  # self.max_bs x 4
            count += 1
    ...
    def run_ft(self, num_fts=4096):
        ...
        responses = np.zeros((num_fts, self.max_h_ind, self.max_w_ind))
        ...
        while True:
            try:
                # t0 = time.time()
                h_ind_, w_ind_, fts_ = self.solver.sess.run(
                    [self.h_indices_, self.w_indices_, self.solver.net.im_b_feat]
                )
                # print time.time() - t0
    ...
    def precomputed_analysis_vote_cls(self, num_fts=4096):
        ...
        while True:
            ...
            a_ind = np.ravel_multi_index(
                patch_a_inds.T, [self.max_h_ind, self.max_w_ind]
            )
            b_ind = np.ravel_multi_index(
                patch_b_inds.T, [self.max_h_ind, self.max_w_ind]
            )

            # t0 = time.time()
            preds_ = self.solver.sess.run(
                self.solver.net.pc_cls_pred,
                feed_dict={
                    self.net.precomputed_features: flattened_features,
            ...
            for i in range(preds_.shape[0]):
                responses[
                    inds[i][0] : (inds[i][0] + spread),
                    inds[i][1] : (inds[i][1] + spread),
                    inds[i][2] : (inds[i][2] + spread),
                    inds[i][3] : (inds[i][3] + spread),
                ] += preds_[i]
                vote_counts[
                    inds[i][0] : (inds[i][0] + spread),
                    inds[i][1] : (inds[i][1] + spread),
