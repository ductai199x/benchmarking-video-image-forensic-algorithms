...
class ExifSolver(object):
    ...
    def setup_net(self, net):
        ...
        if self.init_summary:
            self.train_writer = tf.summary.FileWriter(
                os.path.join("./tb", self.exp_name + "_train"), self.sess.graph
            )
            self.test_writer = tf.summary.FileWriter(
                os.path.join("./tb", self.exp_name + "_test")
            )
            self.setup_summary()
        ...
        if self.checkpoint is not None:
            ...
            try:
                ...
                try:
                    print(
                        "Model and checkpoint did not match, attempting to restore only weights"
                    )
    ...
    def setup_summary(self):
        ...
        if self.use_exif_summary:
            ...
            self.individual_summary = {
                tag: tf.summary.scalar("individual/" + tag, self.tag_holder[tag])
    ...
    def get_data(self, batch_size, split="train"):
        ...
        args = {self.net.im_a: data_dict["im_a"], self.net.im_b: data_dict["im_b"]}
    ...
    def show(self, writer, phase="train"):
        ...
        io.show(
            [["Train time", np.mean(list(self.train_timer))]], phase=phase, iter=self.i
        )
        return
    ...
    def test(self, writer):
        if self.use_exif_summary:
            ...
            print(
                "EXIF test accuracy evaluation took %.2f seconds"
