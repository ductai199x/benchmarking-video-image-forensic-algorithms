...
class EXIFNet:
    ...
    def __init__(
        ...
        self.pc_im_a_feat = tf.map_fn(
            self.mapping_fn, self.im_a_index, dtype=tf.float32, infer_shape=False
        )
        self.pc_im_a_feat.set_shape((None, 4096))
        self.pc_im_b_feat = tf.map_fn(
            self.mapping_fn, self.im_b_index, dtype=tf.float32, infer_shape=False
        )
            ...
            for i, gpu_id in enumerate(self.use_gpu):
                ...
                with tf.device("/gpu:%d" % gpu_id):
                    ...
                    with tf.name_scope("predict_same"):
                        ...
                        pc_feat_ab = tf.concat(
                            [self.pc_im_a_feat, self.pc_im_b_feat], axis=-1
                        )
                    ...
                    if self.train_classifcation:
                        with tf.name_scope("predict_same_image"):
                            if self.use_classify_with_feat:
                                ...
                                pc_cls_out = self.classify_with_feat(
                                    self.pc_im_a_feat,
                                    self.pc_im_b_feat,
                                    pc_out,
                                    name="classify",
    ...
    def predict(self, feat_ab, name, reuse=False):
        with tf.compat.v1.variable_scope(name, reuse=reuse):
            ...
            out = slim.fully_connected(
                out, self.num_classes, activation_fn=None, scope="fc_out"
            )
        return out

    def classify_with_feat(
