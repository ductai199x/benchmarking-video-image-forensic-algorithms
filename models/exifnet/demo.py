...
from .utils import benchmark_utils, util
...
def apply_mask(im, mask):
    mask = scipy.misc.imresize(mask, (im.shape[0], im.shape[1])) / 255.0
...
def dbscan_consensus(results, eps_range=(0.1, 0.5), eps_sample=10, dbscan_sample=4):
    ...
    if best_pred is None:
        ...
        compact_dist_matrix = dist_matrix[
            : len(dist_matrix) // 2, : len(dist_matrix) // 2
        ]
...
def run_vote_no_threads(
    ...
    if exif_to_use is None:
        ...
        not_exif = False
        exif_map = {
            e: np.squeeze(np.argwhere(np.array(solver.net.train_runner.tags) == e))
    ...
    for anchor_idx in range(n_anchors):
        if override_anchor is False:
            _h, _w = np.random.randint(0, h - patch_size), np.random.randint(
                0, w - patch_size
            )
        else:
        ...
        for i in np.linspace(0, h - patch_size, num_per_dim).astype(int):
            for j in np.linspace(0, w - patch_size, num_per_dim).astype(int):
                ...
                if len(batch_b) == batch_size:
                    if not_exif:
                        pred = solver.sess.run(
                            solver.net.cls_pred,
                            feed_dict={
                                solver.net.im_a: batch_a,
                    ...
                    for p_vec, (_i, _j) in zip(pred, batch_b_coord):
                        for e in exif_to_use:
                            ...
                            responses[e][
                                anchor_idx, _i : _i + patch_size, _j : _j + patch_size
                            ] += (p * ones)
                            vote_counts[e][
                                anchor_idx, _i : _i + patch_size, _j : _j + patch_size
                            ] += ones
    ...
    return {
        e: {"responses": (responses[e] / vote_counts[e]), "anchors": anchor_indices}
        for e in exif_to_use
    }
...
class Demo:
    def __init__(
        self,
        ckpt_path="/data/scratch/minyoungg/ckpt/exif_medifor/exif_medifor.ckpt",
        use_gpu=0,
        quality=3.0,
        patch_size=128,
        num_per_dim=30,
        ...
        self.solver, nc, params = load_models.initialize_exif(
            ckpt=ckpt_path, init=False, use_gpu=use_gpu
        )
    ...
    def run(self, im, blue_high=False, use_ncuts=False):
        ...
        out_ms = cv2.resize(
            ms, (im.shape[1], im.shape[0]), interpolation=cv2.INTER_LINEAR
        )
        ...
        for hSt in np.linspace(0, h - patch_size, num_per_dim).astype(int):
            for wSt in np.linspace(0, w - patch_size, num_per_dim).astype(int):
                res = run_vote_no_threads(
                    ...
                    patch_size=128,
                    batch_size=64,
                    sample_ratio=self.quality,
                    override_anchor=(hSt, wSt),
                )["out"]["responses"][0]
...
if __name__ == "__main__":
    ...
    exif_demo = Demo(ckpt_path=ckpt_path, use_gpu=0, quality=1.0, num_per_dim=15)
