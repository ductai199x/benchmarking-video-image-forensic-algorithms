...
from keras.layers import ConvLSTM2D, Conv2D, BatchNormalization
...
def create_featex_vgg16_base(type=1):
    ...
    nb_filters = base
    x = CombinedConv2D(
        32 if type in [0, 1] else 16, activation="relu", use_bias=False, padding="same", name=bname + "c1"
    )(img_input)
    ...
    sf = Conv2DSymPadding(nb_filters, (3, 3), activation=activation, name="transform", padding="same")(x)
...
class GlobalStd2D(Layer):
    ...
    def build(self, input_shape):
        ...
        self.min_std = self.add_weight(
            shape=std_shape, initializer=Constant(self.min_std_val), name="min_std", constraint=non_neg()
        )
        self.built = True
        return
    ...
    def call(self, x):
        ...
        x_std = K.maximum(x_std, self.min_std_val / 10.0 + self.min_std)
        return x_std
...
class NestedWindowAverageFeatExtrator(Layer):
    ...
    def __init__(
        self, window_size_list, output_mode="5d", minus_original=False, include_global=True, **kwargs
    ):
        """
        INPUTS:
    ...
    def _initialize_ii_buffer(self, x):
        x_pad = K.spatial_2d_padding(
            x, ((self.max_wh // 2 + 1, self.max_wh // 2 + 1), (self.max_ww // 2 + 1, self.max_ww // 2 + 1))
        )
...
def create_manTraNet_model(
    ...
    devf5d = NestedWindowAverageFeatExtrator(
        window_size_list=pool_size_list, output_mode="5d", minus_original=True, name="nestedAvgFeatex"
    )(bf)
